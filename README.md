...
AIエージェント「Goose」をはじめ、「OpenCode」や「OpenClaw 2.0」など複数のエージェント（Geese）を安全に実行するための、Dockerベースのネットワーク隔離・監査サンドボックス基盤です。

Docker の `internal: true` ネットワーク（L3/L4）と Squid フォワードプロキシ（L7）の **多層防御** により、エージェント群による未許可の外部通信や意図しないデータ流出を遮断し、全通信試行を構造化 JSON ログに記録・監査します。また、エージェント本体の不要なテレメトリ送信もデフォルトで無効化されています。
